...
# 1 -> 10 -> 250
flags.DEFINE_integer('samples_1', 25, 'number of samples in layer 1') # 底层
# FIXME: users samples 是什么意思？可能是二分图的原因，第一层是物品节点，第二层是用户节点
flags.DEFINE_integer('samples_2', 10, 'number of users samples in layer 2') # 上面一层
...
def evaluate(sess, model, minibatch_iter, size=None):
    ...
    outs_val = sess.run([model.loss, model.ranks, model.mrr], # FIXME: model.ranks是什么意思？
...
def train(train_data, test_data=None):
    G = train_data[0]
    features = train_data[1] # (14755, 50)
    id_map = train_data[2] # 14755

    # features 不为空
    if not features is None:
        # pad with dummy zero vector
        # 这里vstack为features添加列一行0向量，用于WX + b中与b相加。# FIXME: to理解
        # shape: (14755, 50) -> (14756, 50)
        features = np.vstack([features, np.zeros((features.shape[1],))])

    context_pairs = train_data[3] if FLAGS.random_context else None # random walk
    ...
    # 在 Init variables 处， feed， 因为adj_info取决于实际的batch_size
    # shape: (14756, 100)
    # _ph: placeholder
    ...
    if FLAGS.model == 'graphsage_mean':
        # Create model
        sampler = UniformNeighborSampler(adj_info) # adj_info shape: (14756, 100)
        # [SAGEInfo(layer_name='node', neigh_sampler=<graphsage.neigh_samplers.UniformNeighborSampler object at 0x000001AD39FD3AC8>, num_samples=25, output_dim=128),
        # SAGEInfo(layer_name='node', neigh_sampler=<graphsage.neigh_samplers.UniformNeighborSampler object at 0x000001AD39FD3AC8>, num_samples=10, output_dim=128)]
        ...
        sampler = UniformNeighborSampler(adj_info)
        # 对于每一个最终需要求emb的节点，先采样samples_2(10)个邻居节点，
        # 再对这sample_2个邻居节点分别采样sample_1个节点，得到sample_2*sample_1(250)个邻居节点
        layer_infos = [SAGEInfo("node", sampler, FLAGS.samples_1, FLAGS.dim_1),
                            SAGEInfo("node", sampler, FLAGS.samples_2, FLAGS.dim_2)]
    ...
    # Init variables
    sess.run(tf.global_variables_initializer(), feed_dict={adj_info_ph: minibatch.adj}) # minibatch.adj.shape: (14756, 100)
...
def main(argv=None):
    ...
    train_data = load_data(FLAGS.train_prefix, load_walks=True) # G, feats, id_map, walks, class_map
