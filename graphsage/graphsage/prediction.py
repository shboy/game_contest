...
class BipartiteEdgePredLayer(Layer):
    ...
    def affinity(self, inputs1, inputs2):
        """ Affinity(类同) score between batch of inputs1 and inputs2.
        Args:
    ...
    """
        Cross Entropy: 正负样本比例 1: 20
        input1: Tensor("l2_normalize:0", shape=(?, 256), dtype=float32)
        input2: Tensor("l2_normalize_1:0", shape=(?, 256), dtype=float32)
        neg_samples: Tensor("l2_normalize_2:0", shape=(20, 256), dtype=float32)
    """
    def _xent_loss(self, inputs1, inputs2, neg_samples, hard_neg_samples=None):
        aff = self.affinity(inputs1, inputs2) # result = tf.reduce_sum(inputs1 * inputs2, axis=1) shape: (batch_size, )
        neg_aff = self.neg_cost(inputs1, neg_samples, hard_neg_samples) # neg_aff = tf.matmul(inputs1, tf.transpose(neg_samples)) (batch_size, 256).matmul((256, 20)) => (batch_size, 20)
        true_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.ones_like(aff), logits=aff) # 这些都是正样本，所以label都是1 shape: (batch_size, )
        negative_xent = tf.nn.sigmoid_cross_entropy_with_logits(
                labels=tf.zeros_like(neg_aff), logits=neg_aff) # 这些都是负样本， 所以label都是0 (batch_size, 20)
