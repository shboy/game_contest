...
class MaxPoolingAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        neigh_h = tf.reduce_max(neigh_h, axis=1) # max pool
...
class MeanPoolingAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        for l in self.mlp_layers:
            h_reshaped = l(h_reshaped)
        neigh_h = tf.reshape(h_reshaped, (batch_size, num_neighbors, self.hidden_dim))
        neigh_h = tf.reduce_mean(neigh_h, axis=1) # mean pool
...
class SeqAggregator(Layer):
    ...
    def _call(self, inputs):
        ...
        batch_size = dims[0]
        # 用的是 BasicLSTMCell 提供的 zero_state 函数
        initial_state = self.cell.zero_state(batch_size, tf.float32)
        """
            neigh_vecs: (batch_size, seq_len, dim)
            tf.reduce_max(tf.abs(neigh_vecs), axis=2): (batch_size, seq_len) 
            length: (batch_size, )
        """
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            except ValueError: # FIXME:为何会有这个异常
        ...
        """
            tf.range(0, batch_size) * max_len: 
                0, max_len, 2*max_len, ..., (batch_size-1)*max_len
            index:
                0+length_0-1, max_len+(length_1-1), ..., (batch_size-1)*max_len+(length_n-1 - 1)
        """
        index = tf.range(0, batch_size) * max_len + (length - 1)
        flat = tf.reshape(rnn_outputs, [-1, out_size]) # shape: (batch_size*max_len, out_size)
        neigh_h = tf.gather(flat, index) # 选出batch中每一条的实际长度的最后一个output; shape: (batch_size, out_size)
